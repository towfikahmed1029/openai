...
os.chdir(dname)
os.environ['TRANSFORMERS_CACHE']=dname+"/cache"
...
### input type 1 = Test Genaration
### input type 2 = Summaraization
### input type 3 = Pegasus Paraphrase
### input type 4 = Grammer Correction

## Grammer Correction
happy_tt = HappyTextToText("T5", "vennify/t5-base-grammar-correction")
args = TTSettings(num_beams=10, min_length=1, max_length = 200)

## Test Genaration
tx_generator = pipeline("text-generation", model="EleutherAI/gpt-neo-1.3B",framework="pt")


## Summaraization
su_generator = pipeline("summarization",  model="t5-base", tokenizer="t5-base", framework="pt")

## Pegasus Paraphrase
torch_device = 'cuda' if torch.cuda.is_available() else 'cpu'
peg_tokenizer = PegasusTokenizer.from_pretrained('tuner007/pegasus_paraphrase')
peg_model = PegasusForConditionalGeneration.from_pretrained('tuner007/pegasus_paraphrase').to(torch_device)
def get_response(input_text):
    batch = peg_tokenizer([input_text],truncation=True,padding='longest',max_length=60, return_tensors="pt").to(torch_device)
    translated = peg_model.generate(**batch,max_length=60,num_beams=100, num_return_sequences=1, temperature=1.5)
    tgt_text = peg_tokenizer.batch_decode(translated, skip_special_tokens=True)
    return tgt_text

def gpt(prompt,input_type,max_length):
    prompts ="""{0}""".format(prompt)
    if input_type == 1:
        result = tx_generator(prompts, max_length = max_length, do_sample = True, temperature = 0.9)
        result = result[0]["generated_text"]
    elif input_type == 2:
        result = su_generator(prompts, max_length = len(prompt.split(' '))+100,  min_length=10, do_sample = True, temperature = 0.9)
        result = result[0]["summary_text"]
    elif input_type == 3:
        text1 = get_response(prompts)
        result =  text1
    elif input_type == 4:
        text_final_grm ="grammar: {0}".format(prompts)
        result = happy_tt.generate_text(text_final_grm, args=args)
        result = result.text
    else:
        result = "Input Error"
    return result
...
def query_example():
    ...
    final_result = gpt(text,input_type,max_length=max)
    return final_result

# # http://127.0.0.1:5000?max=200&text=How are you?
